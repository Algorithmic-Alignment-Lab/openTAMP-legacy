...
# Information is track by the environment
    ...
    for i in range(conditions):
        ...
        while len(locs) < config['num_objs'] + 1:
            ...
            for j in range(config['num_obs']):
                for n in range(1, len(can_locs)):
                    if valid[n]:
                        obs_locs.append(can_locs[n] + [np.random.uniform(-np.pi, np.pi)])
                        valid[n] = 0
                        for m in range(n+1, len(can_locs)):
                            if not valid[m]: continue
                            if np.linalg.norm(np.array(can_locs[n]) - np.array(can_locs[m])) < obs_spacing:
                                valid[m] = 0
                        break
        ...
        for o in range(config['num_obs']):
            x0[state_inds['box{0}'.format(o), 'pose']] = obs_locs[o][:2]
            x0[state_inds['box{0}'.format(o), 'rotation']] = obs_locs[o][2]
