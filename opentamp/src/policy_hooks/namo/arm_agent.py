...
LOCAL_FRAME = True
NAMO_XML = opentamp.__path__._last_parent_path[1] + '/opentamp' + '/robot_info/lidar_arm.xml'
...
class NAMOArmAgent(NAMOSortingAgent):
    ...
    def run_policy_step(self, u, x):
        ...
        cmd_joint1 = u[self.action_inds['pr2', 'joint1']][0]
        cmd_joint2 = u[self.action_inds['pr2', 'joint2']][0]
        cmd_wrist = u[self.action_inds['pr2', 'wrist']][0]
        nsteps = int(max(abs(cmd_joint1), abs(cmd_joint2)) / self.vel_rat) + 1
    ...
    def get_state(self):
        x = np.zeros(self.dX)
        jnts = env.get_jnt_vec(self.act_jnts)
        for pname, attr in self.state_inds:
            if attr in self.act_jnts:
                x[self.state_inds[pname, attr]] = jnts[self.act_jnts.index(attr)]
            if attr == 'gripper':
                vals = self.mjc_env.get_joints(['left_finger_joint','right_finger_joint'])
                val1 = vals['left_finger_joint']
                val2 = vals['right_finger_joint']
                val = (val1 + val2) / 2.
                x[self.state_inds[pname, attr]] = 0.3 if val > 0 else -0.3
    ...
    def reset_to_state(self, x):
        ...
        self._noops = 0
        self.mjc_env.reset()
        jnts = {jnt: mp_state[self.state_inds['pr2', jnt]] for jnt in self.act_jnts if ('pr2', jnt) in self.state_inds}

        jnts.update('left_finger_joint': x[self.state_inds['pr2', 'gripper']][0])
        jnts.update('right_finger_joint': x[self.state_inds['pr2', 'gripper']][0])
        self.mjv_enc.set_joints(jnts)
    ...
    def retime_traj(self, traj, vel=0.3, inds=None, minpts=10):
        ...
        for step in traj:
            ...
            if inds is None:
                inds = np.r_[\
                             self.state_inds['pr2', 'joint1'], \
                             self.state_inds['pr2', 'joint2'], \
                             self.state_inds['pr2', 'wrist']]
    ...
    def set_symbols(self, plan, task, anum=0, cond=0):
        ...
        if self.task_list[task[0]] == 'moveto':
            ...
            params[3].value[:,0] = params[1].pose[:,st]
        elif self.task_list[task[0]] == 'place':
            params[2].value[:,0] = params[1].pose[:,st]
