...
can0_pose = [-2.0, -4.0]
can1_pose = [3.0, -5.0]
plan.params["pr2"].pose[:, 0] = 0.0
plan.params["pr2"].theta[:, 0] = 0.0
plan.params["pr2"].gripper[:, 0] = -1.0
...
items.append(
    {
        "name": "door",
        "type": "door_2d",
        "handle_dims": (0.325, 0.4),
        "door_dims": (0.75, 0.1, 0.4),
        "hinge_pos": (-1.0, 3.0, 0.5),
        "is_fixed": True,
    }
)
...
for t in range(plan.horizon - 1):
    cur_theta = env.get_joints(["robot_theta"], vec=True)[
        0
    ]  # x[self.state_inds['pr2', 'theta']][0]
    ...
    print(
        t,
        env.get_joints(["door_hinge"]),
        env.get_item_pos("pr2"),
        env.get_item_pos("door"),
    )
