...
from opentamp.src.policy_hooks.utils.load_task_definitions import parse_state
...
can0_pose = np.array(init_poses[ind1]) / 10.0
can1_pose = np.array(init_poses[ind2]) / 10.0
print(can0_pose, can1_pose)

pose3 = np.array(init_poses[ind3]) / 10.0
jnts = np.zeros(
    5
)  # P.calculateInverseKinematics(plan.params['pr2'].openrave_body.body_id, 5, np.r_[pose3, 0.5])
...
for t in range(plan.horizon - 1):
    ...
    print(
        t,
        env.get_item_pos("can0"),
        env.get_item_pos("can1"),
        env.get_item_pos("ee"),
        env.qpos[-2:],
    )
    ...
    print(
        [
            (attr, getattr(plan.params["pr2"], attr)[:, t])
            for attr in ["joint1", "joint2", "wrist"]
        ]
    )
    print("\n\n\n")
