...
problem = parse_problem_config.ParseProblemConfig.parse(
    p_c, domain, env, use_tf=True, sess=None, visual=visual
)
...
for i in range(1):
    params["cloth{}_init_target".format(i)].value[:, 0] = params[
        "cloth{}".format(i)
    ].pose[:, 0]
...
items.append(
    {
        ...
        "type": "box",
        "is_fixed": True,
        "pos": [1.23 / 2 - 0.1, 0, 0.97 / 2 - 0.375 - 0.665],
        "dimensions": [1.23 / 2, 2.45 / 2, 0.97 / 2],
        "rgba": (0, 0.5, 0, 1),
    }
)
config = {
...
for act in plan.actions:
    for t in range(act.active_timesteps[0], act.active_timesteps[1]):
        ...
        print(
            env.get_left_ee_pos(),
            env.get_item_pos("cloth0"),
            base_act[4:7],
            base_act[7],
            plan.params["baxter"].left_ee_pos[:, t],
        )
    import ipdb
